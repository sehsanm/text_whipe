...
def build_model():
    ...
    model.add(layers.Conv2D(16, (3, 3), activation='relu', input_shape=(1444, 2074, 4)))
...
if __name__ == "__main__":
    session = tf.Session()
    model = build_model()
    X, Y = load_data(['./data/wiki'])
    model.fit(X, Y , epochs=1, steps_per_epoch=7 )
